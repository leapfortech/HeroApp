...
public class DocRtuRegisterAction : MonoBehaviour
{
    [Space]
    [Title("Add Doc")]
    [SerializeField]
    ...
    readonly List<Texture2D> docs = new List<Texture2D>();
    ...
    public bool Register()
    {
        ...
        if (!isUpdate)
            investmentService.RegisterDocRTU(investmentDocRequest);
        else
            investmentService.UpdateDocRTU(investmentDocRequest);
    ...
    public void ApplyDoc()
    {
        if (!isUpdate)
        {
            StateManager.Instance.UpdateInvestmentStatus(investmentId, 4);
