...
public class MemoryFeedAction : MonoBehaviour
{
    [Space]
    [Title("Feed")]
    [SerializeField]
    ...
    int selectedIdx = -1;
    readonly MemoryFeed emptyMemoryFeed = new MemoryFeed();
    ...
    public void GetPosts(int startLoopIdx, object userData, int direction)
    {
        ...
        MemoryFeedRequest request = new MemoryFeedRequest
        {
            Chunk = startLoopIdx,
        ...
        memoryService.GetFeed(request);
    }

    public void ApplyPosts(MemoryFeedResponse response)
    {
        if (txtDebug != null)
        ...
        {
            int n = feedCount - response.MemoryFeeds.Count;
            for (int i = 0; i < n; i++)
            {
                int k = (startLoopIdx + i) % loopFeed.ValuesCount;
                UpdateValue(emptyMemoryFeed, loopFeed[k], utcNow);
        ...
        if (response.Direction == 3 && response.MemoryFeeds.Count > 0)
        {
            int dataIndex = (startLoopIdx + feedCount - response.MemoryFeeds.Count) % loopFeed.ValuesCount;
    ...
    public void UpdateValue(MemoryFeed memoryFeed, LoopScrollerValue loopValue, DateTime utcNow)
    {
        ...
        String location = memoryFeed.Country;

        if (!String.IsNullOrWhiteSpace(memoryFeed.State))
            location += ", " + memoryFeed.State;

        String date = memoryFeed.DateTime.Value.ToString("MMMM yyyy", CultureInfo.GetCultureInfo("es-GT"));

        loopValue.GetSprite(0)?.Destroy();
        loopValue.SetSprite(0, memoryFeed.TitleSprite);
        loopValue.SetText(1, $"<line-height=70%>{memoryFeed.Title}");
        loopValue.SetText(2, $"<Size=80%><Color=#025581>{location}</Color>   {date}");
        loopValue.SetText(3, $"<Size=80%>{memoryFeed.ReactionCounts[0] + memoryFeed.ReactionCounts[1] + memoryFeed.ReactionCounts[2] + memoryFeed.ReactionCounts[3]} reacciones");

        for (int i = 0; i < 4; i++)
        {
            loopValue.SetText(4 + i, $"{icnReactions[i]}  {memoryFeed.ReactionCounts[i]}");
            loopValue.SetCheck(i, memoryFeed.ReactionPhraseId == i + 1);
        }
    }
    ...
    public void UpdateDebug(int k, MemoryFeed memoryFeed)
    {
        if (memoryFeed.PublicationDateTime.Year == 1753)
            valueDates[k] = "<color=red>--:--:--:---- : -1</color>";
        else
            valueDates[k] = $"<color=red>{memoryFeed.PublicationDateTime.ToString("HH:mm:ss:ffff")} : {memoryFeed.Title}</color>";
